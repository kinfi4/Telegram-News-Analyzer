...
with open('CHANNEL_REGISTRY.json') as registry_file:
    CHANNEL_REGISTRY = json.load(registry_file)

with open('UKRAINIAN_STOP_WORDS.json') as ukrainian_stop_words_file:
    UKRAINIAN_STOP_WORDS = json.load(ukrainian_stop_words_file)

with open('RUSSIAN_STOP_WORDS.json') as russian_stop_words_file:
